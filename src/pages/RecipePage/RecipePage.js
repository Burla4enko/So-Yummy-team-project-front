...
export default function RecipePage() {
  ...
  const { title, description, time, ingredients, instructions, thumb, owner } =
    recipe.data.recipe;
      ...
      <RecipeWrap>
        <RecipeHeroWrap>
          {width < 768 ? (
            <img
              src={recipeMob}
              srcSet={`${recipeMob} 1x, ${recipeMob2x} 2x`}
              alt="background"
              width="455"
            />
          ) : width < 1440 ? (
            <img
              src={recipeTab}
              srcSet={`${recipeTab} 1x, ${recipeTab2x} 2x`}
              alt="background"
              width="768"
            />
          ) : (
            <img
              src={recipeDesk}
              srcSet={`${recipeDesk} 1x, ${recipeDesk2x} 2x`}
              alt="background"
              width="1440"
            />
          )}
