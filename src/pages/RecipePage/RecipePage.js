...
export default function RecipePage() {
  ...
  useEffect(() => {
    axios.get(`https://soyummy-qk5m.onrender.com/api/recipes/byId/${recipeId}`)
    .then(response => {
  ...
  const handleFavoriteClick = () => {
    if (isFavorite) {
      axios
        .delete(`https://soyummy-qk5m.onrender.com/api/recepes/favorite/${recipeId}`, {
      ...
      axios
        .patch(`https://soyummy-qk5m.onrender.com/api/recipes/favorite/${recipeId}`, {
          withCredentials: true,
        })
        .then(() => {
          setIsFavorite(true);
  ...
  const { title, description, time, ingredients, instructions, thumb, owner } = recipe.data.recipe;
  ...
  <RecipeAddToFavotite onClick={handleFavoriteClick}>
    {isFavorite ? "Delete from favorite" : "Add to favorite"}
      ...
      {/* <AddToFavoriteBtn style={{backgroundColor: 'red'}} /> */}
